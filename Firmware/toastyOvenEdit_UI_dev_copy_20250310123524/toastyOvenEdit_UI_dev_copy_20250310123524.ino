...
void run_reflowprofile(void){
  ...
  // Pre-Heat Cycle
  if (tempC > 260) {  // Emergency shutdown for overheating
    digitalWrite(SSRpin, LOW);   // Turn off the heater
    digitalWrite(alertLEDpin, HIGH);  // Turn on alert LED
    Serial.println("ERROR: Overheat detected! Turning off SSR.");
...
void detect_events(void) {
    push_event = rotary.push();
    rotary_event = rotary.rotate();
}
...
void handle_events(void) {
  ...
  if(is_tc_running){
    ...
    if (!isnan(newTemp) && newTemp > -100) {  // Make sure temp is valid
        tempC = newTemp;
        tempF = (tempC * 1.8) + 32;
    } else {
        Serial.println("ERROR: Thermocouple failure! Shutting off heater.");
        digitalWrite(SSRpin, LOW);  // Turn off heating
        digitalWrite(alertLEDpin, HIGH);  // Alert LED on
        return;
    }
    is_redraw = 1;
}
...
uint8_t mui_draw_current_temp(mui_t *ui, uint8_t msg) {
  if ( msg == MUIF_MSG_DRAW   ) {
      u8g2.setCursor(2,10);
      u8g2.print("set: "); u8g2.print(Setpoint); u8g2.print("C");
      u8g2.setCursor(2, 20);
      u8g2.print("Cur: "); u8g2.print(Input); u8g2.print("C");
